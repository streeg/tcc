...
CRYPTO_LIB_MERGED_LIBSCOUT_PATH = 'crypto-lib-merged-result/libscout/'.freeze
...
CRYPTOGUARD_SARIF_PATH = 'cryptoguard-sarif/'.freeze
...
def extract_and_format_name_sarif(file)
  if is_scout?
    file = file.gsub(LIBSCOUT_PATH, CRYPTOGUARD_SARIF_PATH)
    file.gsub('.log', '.apk-sarif')
  else
    file = file.gsub(LIBRADAR_PATH, CRYPTOGUARD_SARIF_PATH)
